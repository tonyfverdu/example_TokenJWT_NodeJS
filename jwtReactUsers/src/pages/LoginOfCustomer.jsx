...
import '../sass/pagesComponents/LoginOfCustomer.scss'


function LoginOfCustomer() {
  const messageOfComponent = {
    titleFormLogin: 'Login Form',
    titleFormLogout: 'Logout Form',
    titleOfFormular: 'Login of user',
    textLogout: 'Logout',
    textLogin: 'Login',
    titleOfRegisterNewCustomer: 'New with us?  Create a new account',
    textCreateNewCustomer: 'New Account',
    titleDataOfdelivery: 'Data for delivery order'
  }
  const buttonKleinConfiguration = {
    parW: '4rem',
    parH: '2rem',
    parFS: '0.9rem'
  }
        ...
        <div className="contMessageLogin">
          ...
          <div className="contWelcome">
            ...
            <div className="contAnimationSpan">
              <span className="textIsLoggend">{`${userCustomer.treament + ' ' + userCustomer.firstName + ' ' + userCustomer.lastName}. You are already loggend`}</span>
            </div>
          </div>
        </div>
      }
      <header className={`headerLogin ${!isLoggedin ? 'colorGreen' : 'colorRed'}`}>
        <h3 className='titleFormularLogin'>{!isLoggedin ? messageOfComponent.titleFormLogin : messageOfComponent.titleFormLogout}</h3>
      </header>
          ...
          <form className="formLogin" onSubmit={(ev) => handleSubmitForm(ev)}>
            ...
            <div className="contButtons">
              <ButtonKlein
                ...
                text={"Delete"}
                parW={buttonKleinConfiguration.parW}
                parH={buttonKleinConfiguration.parH}
                parFS={buttonKleinConfiguration.parFS}
              />
