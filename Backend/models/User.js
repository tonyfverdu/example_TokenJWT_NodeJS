...
userSchema.statics.findByAuthToken = function (token) {
  console.log({ token, superSecretKey })
  // const userFind = User.findById(decode._id).where('tokens').equals(token);
  const userFind = User.findOne().where('tokens').equals(token);

  return User.findOne().where('tokens').equals(token);
